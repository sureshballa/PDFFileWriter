...
public class OtherExample
	...
	public void CreateTableOfContents()
		...
		Contents.DrawText(ArialBold, 24, 0.5 * PageWidth, PageHeight - Margin - (HeadingHeight - ArialBold.CapHeight(24)) / 2,
			TextJustify.Center, "Video Embedded PDF Resume");
		...
		PosY -= LineHeight + 0.2;
            ...
            //PosY -= LineHeight;
            Contents.DrawTextWithAnnotation(Page, ArialItalic, 14, PosX, PosY, "Page 1 Executive Summary", new AnnotLinkAction("Page1"));
		    PosY -= LineHeight;
            Contents.DrawTextWithAnnotation(Page, ArialItalic, 14, PosX, PosY, "Page 2 Work Experience", new AnnotLinkAction("Page2"));
            PosY -= LineHeight;
            Contents.DrawTextWithAnnotation(Page, ArialItalic, 14, PosX, PosY, "Page 3 Academic Profile", new AnnotLinkAction("Page3"));
            PosY -= LineHeight;
	...
	public void CreatePage5Contents()
		...
		// location markers
		Page.AddLocationMarker("Page2", LocMarkerScope.NamedDest, DestFit.FitH, 10.5);

		// bookmark
		FirstLevelBookmark = BookmarkRoot.AddBookmark("Page 2 Work Experience", Page, 0.0, FirstLevelYPos, 1.0, Color.Red, PdfBookmark.TextStyle.Bold, true);
        ...
        public void CreatePage5_2_Contents()
        {
            // add contents to page
            Contents = AddPageToDocument(3);
            ...
            // bookmark
            FirstLevelBookmark = BookmarkRoot.AddBookmark("Page 3 Academic Profile", Page, 0.0, FirstLevelYPos, 1.0, Color.Red, PdfBookmark.TextStyle.Bold, true);

            // draw examples
            Example5aAcademicProfile(AreaX1, AreaY3);
            //Example5b(AreaX2, AreaY3);
            //Example5c(AreaX1, AreaY2);
            //Example5d(AreaX2, AreaY2);
            //Example5e(AreaX1, AreaY1);
	...
	public void CreatePage7Contents()
		...
		// bookmark
		FirstLevelBookmark = BookmarkRoot.AddBookmark("Page 1 Executive Summary", Page, 0.0, FirstLevelYPos, 1.0, Color.Red, PdfBookmark.TextStyle.Bold, false);

		// draw examples
		//Example7a(AreaX1, AreaY3);
		Example7b(AreaX2 - 2, AreaY3);
	...
	public void Example5a
		...
		{
		// add first level bookmark
		FirstLevelBookmark.AddBookmark("Work Experience", Page, PosX, PosY + AreaHeight, 2.0, true);

		Contents.SaveGraphicsState();
		Contents.Translate(PosX, PosY);
		Double FontSize = 14.0;
		Double BaseLine = AreaHeight - 0.4;
		Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, TextJustify.Left, "Having 4.10 years of experience in software design.");
		BaseLine -= ArialNormal.LineSpacing(FontSize);
		Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, "Extensively worked on software in all the phases including Design.");
        BaseLine -= ArialNormal.LineSpacing(FontSize);
        Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, "Possesses good analytical, logical ability and systematic approach.");
        BaseLine -= ArialNormal.LineSpacing(FontSize);
		Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, "Good Knowledge in C#, .NET, Java Script, TypeScript");
        ...
        public void Example5aAcademicProfile
                (
                Double PosX,
                Double PosY
                )
        {
            // add first level bookmark
            FirstLevelBookmark.AddBookmark("Academic Profile", Page, PosX, PosY + AreaHeight, 2.0, true);

            Contents.SaveGraphicsState();
            Contents.Translate(PosX, PosY);
            Double FontSize = 14.0;
            Double BaseLine = AreaHeight - 0.4;
            Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, TextJustify.Left, "B.Tech Electronics and Communications from XYZ with 71% in the year 2012.");
            BaseLine -= ArialNormal.LineSpacing(FontSize);
            Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, "12th Std from Sri Aurobindo Junior College with 93.5 in the year 2008.");
            BaseLine -= ArialNormal.LineSpacing(FontSize);
            Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, "12th Std from Sri Aurobindo Junior College with 93.5 in the year 2008.");
            BaseLine -= ArialNormal.LineSpacing(FontSize);
            //Contents.DrawText(ArialNormal, FontSize, 0.1, BaseLine, "Good Knowledge in C#, .NET, Java Script, TypeScript");
...
//		Barcode39 Barcode = new Barcode39(new Int32[] {43, 1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 1, 2, 43});
	...
	public void Example7a
		...
		PdfDisplayMedia DisplayMedia = new PdfDisplayMedia(PdfEmbeddedFile.CreateEmbeddedFile(Document, "cb9582bc-a43f-4bb4-91db-faac387ab627.mpeg"));

            DisplayMedia.DisplayControls(false);
...
//		Annotation.ActivateActionWhenPageIsVisible(true);
		...
		// example note
		Contents.DrawText(ArialNormal, NoteSize, PosX + NoteX, PosY + NoteY0, "Play video within the document");
		return;
		}
	...
	public void Example7b
		...
		DisplayMedia.SetMediaWindow(MediaWindow.Floating, 640, 360, WindowPosition.Center,
			WindowTitleBar.TitleBarWithCloseButton, WindowResize.KeepAspectRatio, "Candidate Introduction");
		...
		//Contents.DrawText(ArialNormal, 12.0, TextPosX, TextPosY, TextJustify.Center, "in floating window");
            ...
            // create annotation object
            PdfRectangle AnnotRectText = new PdfRectangle(TextPosX - 0.5 * TextWidth, TextPosY - ArialNormal.DescentPlusLeading(12.0),
            ...
            PdfRectangle AnnotRect7a = ImageSizePos.ImageArea(75, 20, 3*AreaLeft, AreaBottom + 0.8, AreaRight - AreaLeft, AreaTop - AreaBottom, ContentAlignment.MiddleCenter);

            // image resource
            PdfImageControl ImageControl = new PdfImageControl();
            ImageControl.Resolution = 300.0;
            PdfImage Image1 = new PdfImage(Document, "375.png", ImageControl);

            // adjust image size and position
            PdfRectangle AnnotRect = Image1.ImageSizePosition(0.9 * (AreaWidth / 2), 0.9 * ( AreaHeight / 2), ContentAlignment.MiddleCenter);
            //PdfRectangle AnnotRect = new PdfRectangle(PosX, PosY);
            Contents.SaveGraphicsState();
            Contents.DrawImage(Image1, 3*AnnotRect.Width + AnnotRect.Left, 2, AnnotRect.Width, AnnotRect.Height);
            // create annotation object
            PdfAnnotation Annotation = Page.AddScreenAction(AnnotRect7a, DisplayMedia);
            Contents.RestoreGraphicsState();
		...
		// example note
		Contents.DrawText(ArialNormal, NoteSize, PosX + NoteX + 2, PosY + NoteY0, "Play video in a floating window");
		return;
		}
	...
	public void Example7f
		...
		// create display media object
		PdfDisplayMedia Omega = new PdfDisplayMedia(PdfEmbeddedFile.CreateEmbeddedFile(Document, "cb9582bc-a43f-4bb4-91db-faac387ab627.mpeg"));
